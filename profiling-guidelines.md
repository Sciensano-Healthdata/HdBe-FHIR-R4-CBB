...
- When a zib has fundamental changes, the zib is renamed and gets a new id, often by adding a 2 to it (e.g. MedicationAdministration2). The type of a logical model is constructed from the id of the zib. In these cases, the tail of the type and the root element name of the zib logical model differ, which violate the [sdf-8 constraint](https://www.hl7.org/fhir/structuredefinition.html#invs) in FHIR. To fix these violations in CBB logical models, the root element name of the logical models are aligned with the id of the logical model (see also  [zib ticket #1875](https://bits.nictiz.nl/browse/ZIB-1875))).
